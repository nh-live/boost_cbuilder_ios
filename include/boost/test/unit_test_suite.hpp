...
#if BOOST_WORKAROUND(BOOST_EMBTC_FULL_VER, <= 0x030100)
// Workaround for "Test setup error: test tree is empty" issue when linking
// dynamically with Boost Unit Test Framework with Embarcadero's drivers based
// on Clang 3.1.
#define BOOST_AUTO_TU_REGISTRAR( test_name )    \
__attribute__((used)) static boost::unit_test::ut_detail::auto_test_unit_registrar BOOST_JOIN( BOOST_JOIN( test_name, _registrar ), __LINE__ )
#else
#define BOOST_AUTO_TU_REGISTRAR( test_name )    \
static boost::unit_test::ut_detail::auto_test_unit_registrar BOOST_JOIN( BOOST_JOIN( test_name, _registrar ), __LINE__ )
#endif /* BOOST_WORKAROUND(BOOST_EMBTC_FULL_VER, <= 0x030100) */
