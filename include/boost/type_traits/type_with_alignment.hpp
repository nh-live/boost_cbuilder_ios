...
#  if defined(__BORLANDC__) && defined(__clang__)
    // Avoiding the static_assert in the first pass of Clang's templates evaluation
    BOOST_STATIC_ASSERT(sizeof(align_t) > 0 || found >= Align);
    BOOST_STATIC_ASSERT(sizeof(align_t) > 0 || found % Align == 0);
#  else
