...
// If we are using bcc64 in order to compile a C++ program we should use "cmath"
// header, because if "math.h" is included before it will leave out some
// definitions of standard functions that might be needed (i.e. std::abs for
// real types).
#  include <cmath>
#else
#  include <math.h> // So we can check which std C lib we're using
#endif /* defined(__cplusplus) && defined(__BORLANDC__) && defined(__clang__) */
